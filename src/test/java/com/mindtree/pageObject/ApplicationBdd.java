...
public class ApplicationBdd extends downloadappUI {
	WebDriver driver;
	Logger log;
	ExtentTest test;

	public ApplicationBdd(WebDriver driver, Logger log, ExtentTest test) throws Exception {
		this.driver = driver;
		this.test = test;
		this.log = log;
	}
	...
	public void win()throws ReusableComponentException, Exception{
		...
		try {
		   ...
		   if (driver.findElement(playstore).isDisplayed()) {
				ExtentLogUtilities.pass(driver, test, "went to download", log);
				driver.close();
				
			} else {
				ExtentLogUtilities.fail(driver, test, "download error", log);
