...
public class OutstationBdd extends outstationUI {
	WebDriver driver;
	Logger log;
	ExtentTest test;

	public OutstationBdd(WebDriver driver, Logger log, ExtentTest test) throws Exception {
		this.driver = driver;
		this.test = test;
		this.log = log;
	}
	...
	public void verify () throws ReusableComponentException, Exception {
		...
		try {
			if (driver.findElement(verify).isDisplayed()) {
				ExtentLogUtilities.pass(driver, test, "verified uccessfully", log);
				Thread.sleep(2000);
