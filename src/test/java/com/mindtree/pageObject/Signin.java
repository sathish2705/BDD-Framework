...
public class Signin extends SigninUI {
	...
	public void login(String Uid, String Pw) throws ReusableComponentException, Exception {
		WebDriverSupport.sendKeys(driver, email, "Account Log in", "entered Email", log, test, Uid);
		WebDriverSupport.sendKeys(driver, password, "Account Log in", "entered password", log, test, Pw);
	}
	public void clicksignin() throws ReusableComponentException, Exception {
		...
		if (driver.findElement(myacc).isDisplayed()) {
			ExtentLogUtilities.pass(driver, test, "login verified", log);
		} else {
			ExtentLogUtilities.fail(driver, test, "logged failed", log);
