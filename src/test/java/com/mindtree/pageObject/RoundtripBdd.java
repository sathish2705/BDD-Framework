...
public class RoundtripBdd extends roundtripUI {
	WebDriver driver;
	Logger log;
	ExtentTest test;

	public RoundtripBdd(WebDriver driver, Logger log, ExtentTest test) throws Exception {
		this.driver = driver;
		this.test = test;
		this.log = log;
	}
	...
	public void Outstation(String fro) throws ReusableComponentException, Exception {
		...
		WebDriverSupport.sendKeys(driver, to, "to", "place selected.", log, test, too);
	...
	public void verify() throws ReusableComponentException, Exception {
		...
		try {
			...
			if (driver.findElement(bookingdetail).isDisplayed()) {
				ExtentLogUtilities.pass(driver, test, "booking detail is displayed", log);
